...
import { parseK6Output, mergePodMetrics, parseTimeToSeconds } from "./parser.ts";
...
async function main() {
  ...
  for (const competitorConfig of config.competitors) {
    ...
    try {
      ...
      const k8sManifest = generateK8sManifest(competitor, config.resources, competitorConfig.env);
      ...
      for (const testType of config.test.types) {
        ...
        const idleDbMetrics = dbType ? await getPodMetrics(dbType) : { cpu: 0, memory: 0 };
        ...
        const metricInterval = setInterval(async () => {
          ...
          const [m, dbM, c] = await Promise.all([
            ...
            dbType ? getPodMetrics(dbType) : Promise.resolve({ cpu: 0, memory: 0 }),
