...
import {
  generateK8sManifest,
  generatePostgresManifest,
  generateMssqlManifest,
} from "./manifests.ts";
import {
  parseK6Output,
  parseK6Errors,
...
console.log = (...args: any[]) => {
  ...
  if (["verbose", "info", "summary"].includes(currentLogMode))
    originalLog(...args);
};
  ...
  try {
        ...
        containers: [
          {
            ...
            volumeMounts: [
              { name: "k6-script", mountPath: "/scripts", readOnly: true },
            ],
          },
        ],
    ...
    while (Date.now() < deadline) {
      try {
        ...
        phase = (
          await $`timeout 10 kubectl get pod ${podName} -o jsonpath='{.status.phase}'`.text()
        )
          .trim()
          .replace(/'/g, "");
        if (phase === "Succeeded" || phase === "Failed") break;
      } catch {
        /* pod may not exist yet or kubectl timed out */
      }
      ...
      if (elapsed - lastLogTime >= 30) {
        ...
        console.debug(
          `${tag}⏳ k6 pod '${podName}' still running... (${formatDuration(elapsed)} elapsed, phase: ${phase || "Pending"})`,
        );
      }
    ...
    if (phase !== "Succeeded" && phase !== "Failed") {
      throw new Error(
        `Pod ${podName} timed out after ${timeoutSecs}s (last phase: ${phase})`,
      );
    }
    ...
    await $`timeout 10 kubectl delete pod ${podName} --ignore-not-found --force --grace-period=0`
      .quiet()
      .catch(() => {});
    await $`timeout 10 kubectl delete configmap ${configMapName} --ignore-not-found`
      .quiet()
      .catch(() => {});
    try {
...
async function runCompetitor(
  ...
  try {
    ...
    if (isVerbose) {
      await $`docker buildx build --load ${buildArgs} -t ${competitor}:benchmark ./competitors/${competitor}`;
    } else {
      await $`docker buildx build --load ${buildArgs} -t ${competitor}:benchmark ./competitors/${competitor}`.quiet();
    }
    ...
    if (dbType === "postgres") {
      console.info(
        `[${competitor}] ☸️  Deploying PostgreSQL database (${dbServiceName})...`,
      );
      const pgManifest = generatePostgresManifest(
        config.databaseResources,
        dbSuffix,
        config.test.vus,
      );
      ...
      console.info(
        `[${competitor}] ☸️  Deploying MSSQL database (${dbServiceName})...`,
      );
    ...
    if (dbType) {
      ...
      console.debug(
        `[${competitor}] ⏳ Waiting ${settleDurationStr} for database to settle...`,
      );
    ...
    for (const testType of config.test.types) {
      ...
      const warmupPodName = rawWarmupName
        .substring(0, 63)
        .replace(/-+$/, "")
        .toLowerCase();
      ...
      if (warmupSecs > 0) {
        ...
        try {
          ...
          console.warn(
            `[${competitor}] ⚠️ Warmup failed or timed out (ignoring):`,
            e,
          );
        }
      } else {
      ...
      if (idleWaitMs > 0) {
        console.debug(
          `[${competitor}] ⏳ Waiting ${idleWaitDurationStr} for resources to settle before test...`,
        );
        await new Promise((resolve) => setTimeout(resolve, idleWaitMs));
      } else {
        console.debug(
          `[${competitor}] ⏩ Skipping idle wait for '${testType}' (duration is ${idleWaitDurationStr})`,
        );
      }

      // Capture Idle metrics before test
      const idleMetrics = await getPodMetrics(competitor);
      const idleDbMetrics = dbType
        ? await getPodMetrics(dbAppLabel)
        : { cpu: 0, memory: 0 };
      const idleConnections = dbType
        ? await getDbConnectionCount(dbDeploymentName, dbType, competitor)
        : 0;
      console.debug(
        `[${competitor}] 💤 Idle Metrics -> App CPU: ${idleMetrics.cpu}m, App RAM: ${idleMetrics.memory}Mi | DB CPU: ${idleDbMetrics.cpu}m, DB RAM: ${idleDbMetrics.memory}Mi, Connections: ${idleConnections}`,
      );

      // --- ACTUAL TEST PHASE ---
      const rawTestName = `k6-t-${competitor}-${sanitizedTestType}`;
      const testPodName = rawTestName
        .substring(0, 63)
        .replace(/-+$/, "")
        .toLowerCase();
      ...
      console.info(
        `[${competitor}] 🧨 Running load test '${testType}' with 'k6' for ${config.test.duration}...`,
      );
      console.debug(
        `[${competitor}] Executing: k6 run --vus ${config.test.vus} --duration ${testDurationSecs}s -e TARGET_URL=${targetUrl} -e TEST_TYPE=${testType} from inside cluster...`,
      );
        ...
        while (testRunning) {
          try {
            const [m, dbM, c] = await Promise.all([
              ...
              dbType
                ? getPodMetrics(dbAppLabel)
                : Promise.resolve({ cpu: 0, memory: 0 }),
              dbType
                ? getDbConnectionCount(dbDeploymentName, dbType, competitor)
                : Promise.resolve(0),
            ]);
      ...
      try {
        ...
        console.warn(
          `[${competitor}] ⚠️ Load test '${testType}' timed out or failed:`,
          e,
        );
        testOutput = String(e);
      }
    ...
    console.log(`======================================================`);
    console.log(
      `[${competitor}] 🏁 Completed in ${formatDuration(Math.round((Date.now() - competitorStartTime) / 1000))}.`,
    );
    console.log(`======================================================`);
  } catch (error) {
...
async function main() {
  ...
  console.log(
    `Loaded config for ${config.competitors.length} competitors. Test duration: ${config.test.duration}/type.`,
  );
