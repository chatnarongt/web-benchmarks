...
new Elysia()
  ...
  .get("/database/single-write", async ({ query }) => {
    ...
    return await sql.begin(async (tx) => {
      const [world] = await tx`SELECT id, randomnumber AS "randomNumber" FROM World WHERE id = ${id}`;
      if (world) {
        const newRandomNumber = +(query.randomNumber as string) || 1;
        await tx`UPDATE World SET randomNumber = ${newRandomNumber} WHERE id = ${id}`;
  ...
  .get("/database/multiple-write", async ({ query }) => {
    ...
    return await sql.begin(async (tx) => {
      const worlds = await tx`SELECT id, randomnumber AS "randomNumber" FROM World LIMIT ${limit} OFFSET ${offset}`;

      if (worlds && worlds.length > 0) {
        const sortedWorlds = [...worlds].sort((a, b) => a.id - b.id);

        const values = sortedWorlds.map((w: any) => {
          const originalIndex = worlds.findIndex((orig: any) => orig.id === w.id);
          w.randomNumber = rVals[originalIndex] || 1;
          return `(${w.id}, ${w.randomNumber})`;
        }).join(', ');

        await tx.unsafe(`UPDATE World SET randomnumber = u.rn FROM (VALUES ${values}) AS u(id, rn) WHERE World.id = u.id`);
      }
